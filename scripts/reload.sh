...
APP_NAME="Kyu-mux DEV"
BUNDLE_ID="com.kyumux.app.debug"
BASE_APP_NAME="Kyu-mux DEV"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
CLI_PATH_FILE="/tmp/kyumux-last-cli-path"
...
select_cmux_shim_target() {
  local app_cli_dir="/Applications/Kyu-mux.app/Contents/Resources/bin"
...
tagged_derived_data_path() {
  local slug="$1"
  echo "$HOME/Library/Developer/Xcode/DerivedData/kyumux-${slug}"
}
...
print_tag_cleanup_reminder() {
  ...
  while IFS= read -r -d '' path; do
    if [[ "$path" == /tmp/kyumux-* ]]; then
      tag="${path#/tmp/kyumux-}"
    elif [[ "$path" == "$HOME/Library/Developer/Xcode/DerivedData/kyumux-"* ]]; then
      tag="${path#$HOME/Library/Developer/Xcode/DerivedData/kyumux-}"
    else
      continue
    fi
    ...
    find /tmp -maxdepth 1 -name 'kyumux-*' -print0 2>/dev/null
    find "$HOME/Library/Developer/Xcode/DerivedData" -maxdepth 1 -type d -name 'kyumux-*' -print0 2>/dev/null
  )
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"Kyu-mux DEV ${tag}.app/Contents/MacOS/Kyu-mux DEV\""
      echo "  rm -rf \"$(tagged_derived_data_path "$tag")\" \"/tmp/kyumux-${tag}\" \"/tmp/kyumux-debug-${tag}.sock\""
      echo "  rm -f \"/tmp/kyumux-debug-${tag}.log\""
      echo "  rm -f \"$HOME/Library/Application Support/cmux/kyumuxd-dev-${tag}.sock\""
    done
  fi
  echo "After you verify current tag, cleanup command:"
  echo "  pkill -f \"Kyu-mux DEV ${current_slug}.app/Contents/MacOS/Kyu-mux DEV\""
  echo "  rm -rf \"$(tagged_derived_data_path "$current_slug")\" \"/tmp/kyumux-${current_slug}\" \"/tmp/kyumux-debug-${current_slug}.sock\""
  echo "  rm -f \"/tmp/kyumux-debug-${current_slug}.log\""
  echo "  rm -f \"$HOME/Library/Application Support/cmux/kyumuxd-dev-${current_slug}.sock\""
}
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="Kyu-mux DEV ${TAG_SLUG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.kyumux.app.debug.${TAG_ID}"
  fi
...
RELOAD_LOG="/tmp/kyumux-reload-${TAG_SLUG}.log"
...
reload_finalize() {
  ...
  if [[ -x "${CLI_PATH:-}" ]]; then
    ...
    echo "CLI helpers:"
    echo "  /tmp/kyumux-cli ..."
    echo "  $HOME/.local/bin/kyumux-dev ..."
...
if [[ -n "${TAG_SLUG:-}" ]]; then
  TMP_COMPAT_DERIVED_LINK="/tmp/kyumux-${TAG_SLUG}"
...
if [[ -n "$TAG" && "$APP_NAME" != "$SEARCH_APP_NAME" ]]; then
  ...
  if [[ -f "$INFO_PLIST" ]]; then
    ...
    if [[ -n "${TAG_SLUG:-}" ]]; then
      ...
      CMUXD_SOCKET="${APP_SUPPORT_DIR}/kyumuxd-dev-${TAG_SLUG}.sock"
      CMUX_SOCKET_PATH_VALUE="/tmp/kyumux-debug-${TAG_SLUG}.sock"
      CMUX_DEBUG_LOG="/tmp/kyumux-debug-${TAG_SLUG}.log"
      write_last_socket_path "$CMUX_SOCKET_PATH_VALUE"
      echo "$CMUX_DEBUG_LOG" > /tmp/kyumux-last-debug-log-path || true
...
if [[ -x "$CLI_PATH" ]]; then
  (umask 077; printf '%s\n' "$CLI_PATH" > /tmp/kyumux-last-cli-path) || true
  ln -sfn "$CLI_PATH" /tmp/kyumux-cli || true

  # Stable shim that always follows the last reload-selected dev CLI.
  DEV_CLI_SHIM="$HOME/.local/bin/kyumux-dev"
  write_dev_cli_shim "$DEV_CLI_SHIM" "/Applications/Kyu-mux.app/Contents/Resources/bin/cmux"

  CMUX_SHIM_TARGET="$(select_cmux_shim_target || true)"
  if [[ -n "${CMUX_SHIM_TARGET:-}" ]]; then
    write_dev_cli_shim "$CMUX_SHIM_TARGET" "/Applications/Kyu-mux.app/Contents/Resources/bin/cmux"
  fi
fi
...
if [[ -x "$CLI_PATH" ]]; then
  echo "$CLI_PATH" > /tmp/kyumux-last-cli-path || true
fi
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  if [[ -n "${TAG_SLUG:-}" && -n "${CMUX_SOCKET_PATH_VALUE:-}" ]]; then
    ...
    echo "/tmp/kyumux-debug.sock" > /tmp/kyumux-last-socket-path || true
    echo "/tmp/kyumux-debug.log" > /tmp/kyumux-last-debug-log-path || true
